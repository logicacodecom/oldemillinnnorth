...
// Contact handler: never accepts-and-drops a message.
...
export async function POST(request: Request) {
  ...
  if (resendKey) {
    const from = process.env.RESEND_FROM || "Olde Mill Inn North Website <onboarding@resend.dev>";
    const text = [
      ...
      `Email: ${email}`,
      phone ? `Phone: ${phone}` : "",
      arrival ? `Move-in: ${arrival}` : "",
      departure ? `Move-out: ${departure}` : "",
      "",
      "Message:",
      message,
    ]
  ...
  if (webhook) {
    try {
      const res = await fetch(webhook, {
        ...
        body: JSON.stringify({ ...body, source: "oldemillinnnorth.com/contact" }),
      });
